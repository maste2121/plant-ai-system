...
const AdminDashboard = () => {
    ...
    const fetchDashboardMetrics = async () => {
      try {
        ...
        if (statsRes.data.success && statsRes.data.stats) {
          setLiveStats({
            totalScans:
              statsRes.data.stats.totalScans || statsRes.data.totalScans,
            activeUsers:
              statsRes.data.stats.totalUsers || statsRes.data.activeUsers,
        ...
        const scansRes = await axios.get(`${API_URL}/api/admin/scans`, {
          headers,
        });
  ...
  const statsConfig = [
    ...
    {
      label: "Common Disease",
      value: liveStats.commonDisease,
      icon: <Leaf size={24} />,
      color: "text-red-600",
      bg: "bg-red-50",
    },
      ...
      <header className="flex justify-between items-center mb-10">
        <div>
          ...
          <p className="text-slate-500 text-sm italic">
            Monitoring crop health and farmer activity across Ethiopia.
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-50 flex items-center justify-between">
          <h3 className="font-bold text-slate-800 flex items-center gap-2">
            <Activity size={18} className="text-blue-500" /> Recent Live
            Detections
          </h3>
        </div>
