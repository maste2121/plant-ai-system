...
const ScanLog = () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        const cropsInLogs = res.data.data
          ...
          .filter(
            (cropName, index, self) =>
              cropName && self.indexOf(cropName) === index
          );
        setUniqueCrops(cropsInLogs);
      }
    } catch (err) {
      console.error(
        "Failed parsing live relational scan database matrices:",
        err
      );
    } finally {
      setLoading(false);
  ...
  const filteredScans = scans.filter((scan) => {
    ...
    const matchesCropFilter =
      selectedCropFilter === "" || scan.crop_name === selectedCropFilter;
  ...
  const exportLogsToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `AI_Scan_Audit_Report_${new Date().toISOString().split("T")[0]}.csv`
    );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 font-medium mt-0.5">
            Monitor real-time AI computer vision detections across rural farms.
          </p>
        </div>
        ...
        {loading ? (
          ...
          filteredScans.map((scan) => {
            ...
            return (
              <div
                key={scan.id}
                className="bg-white p-4 rounded-xl border border-slate-200/80 hover:border-emerald-200 shadow-xs hover:shadow-md transition-all flex flex-col md:flex-row md:items-center gap-6"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] font-semibold text-slate-400 mt-1.5 bg-slate-100 w-fit px-2 py-0.5 rounded-md">
                    {new Date(scan.scan_date).toLocaleDateString()} • SCN-
                    {scan.id}
                  </p>
                </div>
                  ...
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span
                      className={`text-xs font-bold ${
                        isConfident ? "text-emerald-600" : "text-amber-500"
                      }`}
                    >
                      {confidence}%
                    </span>
                  ...
                  <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden border border-slate-200/40">
                    <div
                      className={`h-full rounded-full transition-all duration-500 ${
                        isConfident ? "bg-emerald-500" : "bg-amber-400"
                      }`}
