...
const Settings = () => {
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(
        `${API_URL}/api/admin/profile/update`,
        { email: adminInfo.email },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (res.data.success) {
        setStatus({
          text: res.data.message || "Profile email updated successfully.",
          isError: false,
          context: "profile",
        });
      }
      ...
      const errorMsg =
        ...
        "Failed updating profile coordinates.";
  ...
  const handleSecuritySubmit = async (e) => {
    ...
    if (passwordData.newPassword !== passwordData.confirmPassword) {
      setStatus({
        text: "New password confirmations do not match.",
        isError: true,
        context: "security",
      });
      return;
    ...
    try {
      ...
      if (res.data.success) {
        setStatus({
          text: res.data.message || "Authentication keys updated successfully.",
          isError: false,
          context: "security",
        });
        setPasswordData({
      ...
      const errorMsg =
        err.response?.data?.message ||
        err.response?.data?.msg ||
        "Invalid current credential block.";
    ...
    <div className="p-8 lg:p-12 bg-slate-50 min-h-screen">
      ...
      <div className="max-w-5xl mx-auto mb-10">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Manage core admin accounts, profile communication logs, and
          cryptographic safety matrices.
        </p>
      </div>
        ...
        <div className="lg:col-span-1 space-y-2">
          ...
          >
            <User
              size={18}
              className={
                activeTab === "profile" ? "text-emerald-600" : "text-slate-450"
              }
            />
            Profile Properties
          </button>
          ...
          >
            <Shield
              size={18}
              className={
                activeTab === "security" ? "text-emerald-600" : "text-slate-450"
              }
            />
            Security & Keys
          </button>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    Update the master email identity linked to this
                    administrative node panel.
                  </p>
                </div>
                ...
                <form onSubmit={handleProfileSubmit} className="space-y-5 pt-2">
                  ...
                  <div className="max-w-lg">
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                      Master Superuser Email Address
                    </label>
                    <input
                      type="email"
                      required
                      ...
                      } // Fixed: Added real mutation input binding
                  ...
                  <div className="pt-4 border-t border-slate-100 flex justify-end">
                    ...
                    >
                      <Save size={14} />{" "}
                      {loading ? "Saving Changes..." : "Save Profile Email"}
                    </button>
                  </div>
                </form>
              </div>
            ...
            {activeTab === "security" && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    Rotate operational cryptographic authorization keys safely
                    to protect agricultural scan records.
                  </p>
                </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
                    ...
                    <div>
                      ...
                      <input
                        type="password"
                        required
                        placeholder="Repeat chosen characters"
